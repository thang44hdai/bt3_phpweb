...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["endDate"])) {
        $endDateErr = "Vui lòng chọn ngày bắt đầu đi làm";
    } else {
...
if (isset($_POST['submit'])) {
    if (!empty($reason) && !empty($detail) && !empty($startDate) && !empty($endDate)) {
        $id = $_GET['id'];
        $query2 = "INSERT INTO nghi_phep(id_nhanvien, ly_do, chi_tiet, ngay_bat_dau, ngay_ket_thuc, ngay_tao_don, trang_thai)
